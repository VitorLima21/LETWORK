<?php
	...
	$stmt = $conn->prepare('SELECT produto.nome, produto.valor, produto.imagem, carrinhodeproduto.quantidade
        FROM finalizacompra 
        INNER JOIN carrinho ON carrinho.idcarrinho = finalizacompra.carrinho_idcarrinho 
        INNER JOIN carrinhodeproduto ON carrinho.idcarrinho = carrinhodeproduto.carrinho_idcarrinho 
        INNER JOIN produto ON produto.idproduto = carrinhodeproduto.produto_idproduto 
    ...
    $stmt3 = $conn->prepare('DELETE FROM carrinhodeproduto WHERE carrinho_idcarrinho = :carrinho_idproduto');
